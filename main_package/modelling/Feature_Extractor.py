...
class Feature_Extractor():
    ...
    Quora_word2vec = gensim.models.KeyedVectors.load_word2vec_format('google.news.en.bin', binary=True)
    def Data_Cleaning(self,data):
        ...
        corpus_raw['question1'] = corpus_raw['question1'].apply(
            lambda row: "".join([s if s.isalpha() or s.isdigit() or s == "'" or s == " " else ' ' for s in row]))
        corpus_raw['question2'] = corpus_raw['question2'].apply(
            lambda row: "".join([s if s.isalpha() or s.isdigit() or s == "'" or s == " " else ' ' for s in row]))
    ...
    def Feature_set1(self,corpus_raw):
    ...
    def Feature_set2(self,corpus_raw):
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------#
    def words_to_vectors(self,fs1, filename):
        ...
        q1vector = np.zeros(shape=(fs1.shape[0], self.num_features))
        q2vector = np.zeros(shape=(fs1.shape[0], self.num_features))

        for index, row in fs1.iterrows():
            # Words found in vocabulary
            #Quora_word2vec = ['kids', 'sdf', 'dgsg']
            q1_word = (list)(filter(lambda word: word in self.Quora_word2vec.vocab, row['question1_tokens'].split(',')))
            q2_word = (list)(filter(lambda word: word in self.Quora_word2vec.vocab, row['question2_tokens'].split(',')))

            q1_vector, q2_vector = self.get_vector(q1_word, q2_word, self.Quora_word2vec, self.num_features)
            q1vector[index] = q1_vector
            q2vector[index] = q2_vector
            self.Distance_Functions(self.Quora_word2vec, fs1, index, q1_vector, q2_vector, row['question1_tokens'],
                               row['question2_tokens'], self.num_features)
        ...
        self.Normalized_Word_Mover_Distance(fs1, self.Quora_word2vec)

        # Write into file here
        self.Write_to_csv(fs1, Features, self.num_features, filename)


    def get_vector(self,q1_word, q2_word, model, num_features):
        q1_vector_word = []
        q2_vector_word = []
    ...
    def Distance_Functions(self,model, fs1, index, q1_vector, q2_vector, q1, q2, num_features):
        try:
            fs1.set_value(index, 'euclidean', self.Euclidean_Distance(q1_vector, q2_vector))
        ...
        try:
            fs1.set_value(index, 'manhattan', self.Manhattan(q1_vector, q2_vector))
        except Exception as e:

            fs1.set_value(index, 'manhattan', np.nan)
        try:
            fs1.set_value(index, 'jaccard', self.Jaccard(q1_vector, q2_vector))
        except Exception as e:

            fs1.set_value(index, 'jaccard', np.nan)
        try:
            fs1.set_value(index, 'cossimilarity', self.Cosine_Similarity(q2_vector, q2_vector))
        except Exception as e:

            fs1.set_value(index, 'cossimilarity', np.nan)
        try:
            fs1.set_value(index, 'canberra', self.Canberra(q1_vector, q2_vector))
        except Exception as e:

            fs1.set_value(index, 'canberra', np.nan)
        try:
            fs1.set_value(index, 'braycurtis', self.Braycurtis(q1_vector, q2_vector))
        except Exception as e:

            fs1.set_value(index, 'braycurtis', np.nan)
        try:
            fs1.set_value(index, 'minkowski', self.Minkowksi(q1_vector, q2_vector, 3))
        except Exception as e:

            fs1.set_value(index, 'minkowski', np.nan)
        try:
            fs1.set_value(index, 'wordmover', self.Word_Mover_Distance(q1, q2, model))
        except Exception as e:

            fs1.set_value(index, 'wordmover', np.nan)
        try:
            fs1.set_value(index, 'kurtosisq1', self.Kurtosis_q1_vector(q1_vector, num_features))
        except Exception as e:

            fs1.set_value(index, 'kurtosisq1', np.nan)
        try:
            fs1.set_value(index, 'kurtosisq2', self.Kurtosis_q2_vector(q2_vector, num_features))
        except Exception as e:

            fs1.set_value(index, 'kurtosisq2', np.nan)
        try:
            fs1.set_value(index, 'skewq1', self.Skew_q1_vector(q1_vector, num_features))
        except Exception as e:

            fs1.set_value(index, 'skewq1', np.nan)
        try:
            fs1.set_value(index, 'skewq2', self.Skew_q2_vector(q2_vector, num_features))
        except Exception as e:
    ...
    def Write_to_csv(self,fs1, Features, num_feaures, filename):
        ...
        if filename == 'train.csv':
            ...
            fs1.to_csv("Quora_Features_" + str(filename), columns=Features, index=False)
    ...
    def Jaccard(self,q1_vector, q2_vector):
        return spatial.distance.jaccard(q1_vector, q2_vector)


    def Kurtosis_q1_vector(self,q1_vector, num_features):
        kurt_q1 = stats.kurtosis(q1_vector.reshape(num_features, 1))[0]
        return kurt_q1


    def Kurtosis_q2_vector(self,q2_vector, num_features):
    ...
    def Skew_q2_vector(self,q2_vector, num_features):
    ...
    def Normalized_Word_Mover_Distance(self,fs1, model):
